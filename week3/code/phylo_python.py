...
def upgma(D):
    ...
    while active_count > 1:
        ...
        left_len = max(0.0, new_height - heights[best_i])
        right_len = max(0.0, new_height - heights[best_j])

        parent = TreeNode(
            [nodes[best_i], nodes[best_j]],
            [left_len, right_len],
            -1
        )
...
def neighbor_joining(dist):
    ...
    import numpy as np
    dist = np.array(dist, dtype=float).tolist()  
    ...
    while len(active_ids) > 2:
        ...
        best_i, best_j, best_q = -1, -1, float("inf")
        for i in range(m):
