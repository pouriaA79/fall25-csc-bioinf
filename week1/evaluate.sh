...
for ds in "${datasets[@]}"; do
  ...
  if [[ -f "$PY_ENTRY" ]]; then
    ...
    py_fa="$(find_best_fasta_in_ds "$ds")"
    py_n50="$(n50_of_fasta "${py_fa:-/dev/null}")"
    echo -e "$ds_name\tpython\t$(fmt_duration "$py_rt")\t${py_n50:-NA}"
    echo "$ds_name,python,$py_rt,$py_n50" >> "$RESULTS_CSV"
  else
    echo -e "$ds_name\tpython\t0:00:00\tNA"
    echo "$ds_name,python,0,NA" >> "$RESULTS_CSV"
  fi
  ...
  if [[ -f "$CODON_ENTRY" ]]; then
    ...
    co_fa="$(find_best_fasta_in_ds "$ds")"
    co_n50="$(n50_of_fasta "${co_fa:-/dev/null}")"
    echo -e "$ds_name\tcodon\t$(fmt_duration "$co_rt")\t${co_n50:-NA}"
    echo "$ds_name,codon,$co_rt,$co_n50" >> "$RESULTS_CSV"
  else
    echo -e "$ds_name\tcodon\t0:00:00\tNA"
    echo "$ds_name,codon,0,NA" >> "$RESULTS_CSV"
  fi
done
